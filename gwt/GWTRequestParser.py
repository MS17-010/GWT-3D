...
PRIMITIVES_WRAPPER = [STRING_OBJECT, INTEGER_OBJECT, DOUBLE_OBJECT,
                      FLOAT_OBJECT, BYTE_OBJECT, BOOLEAN_OBJECT, SHORT_OBJECT,
                      CHAR_OBJECT]
...
NUMERICS = [INT, CHAR, BOOLEAN, BYTE, SHORT, INTEGER_OBJECT, CHAR_OBJECT,
            BYTE_OBJECT, BOOLEAN_OBJECT, SHORT_OBJECT]
...
class GWTReqParser(object):
    def __init__(self, burp, replace, surround, verbose, debug):
        self.burp = burp
        self.surround_value = surround
    ...
    def _parse(self):
        self.rpc_deserialized = list()
        self.parameters = list()  # Stores Parameter names and values read in from the request
